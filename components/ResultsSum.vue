...
<template>
  ...
  <li v-if="resultsSum.matches.total.win + resultsSum.matches.total.lose > 0">
    <span class="muted">通算: </span>
    <MatchSum :result="resultsSum.matches.total" />
    <ul>
      <li v-if="resultsSum.matches.official.win + resultsSum.matches.official.lose > 0">
        <span class="muted">公式大会: </span>
        <MatchSum :result="resultsSum.matches.official" />
      </li>
      <li v-if="resultsSum.matches.unofficial.win + resultsSum.matches.unofficial.lose > 0">
        <span class="muted">非公式大会: </span>
