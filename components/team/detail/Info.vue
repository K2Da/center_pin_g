...
const resultsSum: ComputedRef<Result|undefined> = computed(() => {
  if (detail) {
    return countTournamentResult(
      ...
      detail.tournaments.map((t) => ({ key: t.tournament_key, result: t.result })),
      tournaments.value,
    );
  }
});

const sortedPlayers = (players: string[], playerStats: Record<string, { tournament: number, count: number }>) => {
  return players.slice().sort((a, b) => playerStats[b].count - playerStats[a].count);
};
...
<template>
  <ul>
    ...
    <li>
      ...
      <DateSpan :date1="detail.team.first_tournament_date" :date2="detail.team.latest_tournament_date" />
    </li>
    <ResultsSum :resultsSum="resultsSum" />
    <li><span class="muted">メンバー(出場回数): </span>
        <span
          v-for="(p, i) of sortedPlayers(detail.players, detail.playerStats)"
          :key="p"
        >
          <template v-if="i !== 0">, </template>
          <PlayerName :name="p" /><span class="muted">({{ detail.playerStats[p].count }})</span>
        </span>
    </li>
  </ul>
</template>